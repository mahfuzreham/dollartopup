...
function tr($lang,$bn,$en){return $lang==='en'?$en:$bn;}
function userLang(PDO $db,$uid){$s=$db->prepare('SELECT language FROM telegram_users WHERE telegram_user_id=?');$s->execute([$uid]);return $s->fetchColumn()==='en'?'en':'bn';}
function menuT($t,$c,$x,$lang='bn') {$url='https://api.telegram.org/bot'.$t.'/sendMessage';$d=['chat_id'=>$c,'text'=>$x,'parse_mode'=>'HTML','reply_markup'=>json_encode(['keyboard'=>[[['text'=>$lang==='en'?'💳 Buy Dollar':'💳 ডলার কিনুন'],['text'=>$lang==='en'?'👤 My Profile':'👤 আমার প্রোফাইল']],[['text'=>$lang==='en'?'📊 Last 15 Days':'📊 গত ১৫ দিনের হিস্ট্রি'],['text'=>$lang==='en'?'🆘 Support Team':'🆘 সাপোর্ট টিম']],[['text'=>$lang==='en'?'🌐 Language':'🌐 ভাষা']]],'resize_keyboard'=>true])];if(function_exists('curl_init')){$h=curl_init($url);curl_setopt_array($h,[CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>$d,CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>15]);$r=curl_exec($h);curl_close($h);return $r;}return null;}
function sendK($t,$c,$x,$kb){$url='https://api.telegram.org/bot'.$t.'/sendMessage';$d=['chat_id'=>$c,'text'=>$x,'parse_mode'=>'HTML','reply_markup'=>json_encode(['keyboard'=>$kb,'resize_keyboard'=>true,'one_time_keyboard'=>true])];if(function_exists('curl_init')){$h=curl_init($url);curl_setopt_array($h,[CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>$d,CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>15]);$r=curl_exec($h);curl_close($h);return $r;}return null;}
...
if(!$isAdmin){
 ...
 elseif($text==='/support'||$text==='🆘 Support Team'||$text==='🆘 সাপোর্ট টিম'){$info=gv($db,'support_team','Support team is not configured yet.');sendT($token,$chat,"🆘 <b>Support Team</b>\n\n".htmlspecialchars($info));}
  ...
  elseif($step==='trxid'){if(strlen($text)<3||strlen($text)>100){sendT($token,$chat,'সঠিক TrxID দিন।');}else{$d['trxid']=$text;sess($db,$uid,$chat,'network',$d);sendK($token,$chat,'🌐 <b>Select Network</b>\n\nআপনার USDT receive করার network নির্বাচন করুন।',[[['text'=>'🟡 USDT (BEP20)']]]);}}
  elseif($step==='network'){if($text!=='🟡 USDT (BEP20)'&&strtolower($text)!=='bep20'&&strtolower($text)!=='usdt bep20'){sendK($token,$chat,'⚠️ অনুগ্রহ করে নিচের <b>USDT (BEP20)</b> network নির্বাচন করুন।',[[['text'=>'🟡 USDT (BEP20)']]]);}else{$d['network']='USDT (BEP20)';sess($db,$uid,$chat,'address',$d);sendT($token,$chat,'🟡 <b>Network: USDT (BEP20)</b>\n\nএখন আপনার <b>BEP20 Wallet Address</b> দিন।\n<code>0x...</code>');}}
