...
$db->exec("CREATE TABLE IF NOT EXISTS withdrawal_requests (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, order_no VARCHAR(100) NOT NULL, destination_address VARCHAR(64) NOT NULL, amount DECIMAL(18,8) NOT NULL, status VARCHAR(30) NOT NULL DEFAULT 'queued', tx_hash VARCHAR(66) NULL, verification_error VARCHAR(255) NULL, binance_withdraw_id VARCHAR(100) NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY uniq_withdrawal_order(order_no))");
col($db,'withdrawal_requests','binance_withdraw_id','binance_withdraw_id VARCHAR(100) NULL');
col($db,'withdrawal_requests','tx_hash','tx_hash VARCHAR(128) NULL');
col($db,'telegram_users','language',"language VARCHAR(5) NOT NULL DEFAULT 'bn'");
